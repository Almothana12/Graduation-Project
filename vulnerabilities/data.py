...
def check(session: requests.Session, url: str, sig=None, stop=None) -> None:
    if stop:
        if stop():
            sig.finished.emit()
            return
    # A list of all phones detected on a page
    phones = []
    ...
    for match in iterator:
        ...
        log.warning(f"phone number found: {phone_number} on {url}")
    ...
    #Saudi Arabia. 10 digits starting with 05 or 01.
    phone_regex = r"\b(01|05)\d{8}\b"
    ...
    for match in iterator:
        ...
        phones.append(phone_number)
        add_vulnerability("Phone Number", url, data=match.group())
        log.warning(f"phone number found FR: {match.group()} on {url}")
    ...
    for match in iterator:
        ...
        phones.append(phone_number)
        add_vulnerability("Phone Number", url, data=match.group())
        log.warning(f"phone number found UK : {match.group()} on {url}")
    ...
    #     phones.append(phone_number)
    #     add_vulnerability("Phone Number", url, data=match.group())
    #     log.warning(f"phone number found IND: {match.group()} on {url}")
    ...
    for match in iterator:
        ...
        phones.append(phone_number)
        add_vulnerability("Phone Number", url, data=match.group())
        log.warning(f"phone number found US: {match.group()} on {url}")
    ...
    for match in iterator:
        ...
        phones.append(phone_number)
        add_vulnerability("Phone Number", url, data=match.group())
        log.warning(f"phone number found P: {match.group()} on {url}")
...
if __name__ == "__main__":
    ...
    # match a string that starts with '+' then 10-15 numbers:
    phone_regex = r"\+\d{12}\b"
    # find all matches
    iterator = re.finditer(phone_regex, page)
    # loop through the matches
    for match in iterator:
        phone_number = match.group()
        log.warning(f"phone number found: {phone_number}")
